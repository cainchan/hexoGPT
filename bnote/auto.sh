...
docker run -i --rm --volume $PWD:/data:rw --volume $HOME:/root:rw --workdir /data hkccr.ccs.tencentyun.com/zhengkai/hexo:20-alpine hexo new springboot -s $filename $title
...
#cat springboot-title.list| while read line; do ./auto.sh $line; done